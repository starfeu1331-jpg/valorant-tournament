...
interface PlayerReplyFormProps {
  conversationId: string
  action: (formData: FormData) => Promise<void>
}

export function PlayerReplyForm({ conversationId, action }: PlayerReplyFormProps) {
  ...
  const handleTyping = () => {
    console.log('⌨️ PLAYER handleTyping called')
    if (!isTyping) {
      setIsTyping(true)
      console.log('📤 PLAYER Sending typing=true to API')
      fetch(`/api/staff-conversations/${conversationId}/typing`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isTyping: true }),
      }).then(r => console.log('✅ PLAYER API response:', r.status))
    }
